...
public class BagpipeItem extends Item{
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        ...
        if(!MagicalInstrumentCommonConfig.areaEffect.get()) {
            player.addEffect(new MobEffectInstance(MobEffects.DAMAGE_BOOST, duration, bufflevel));
        } else {
            ...
            for(Player el : (level.getEntitiesOfClass(Player.class, AABB.of(new BoundingBox(pos.getX()-10,pos.getY()-2,pos.getZ()-10,pos.getX()+10,pos.getY()+2,pos.getZ()+10))))) {
                el.addEffect(new MobEffectInstance(MobEffects.DAMAGE_BOOST, duration, bufflevel));
            }
        }
