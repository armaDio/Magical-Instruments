...
public class Horn extends Item {
    ...
    public ItemStack onItemRightClick(ItemStack p_77659_1_, World world, EntityPlayer player) {
        if(!MinstConfiguration.areaEffect) {
            switch(tier){
                case 1:
                    player.addPotionEffect(new PotionEffect(Potion.resistance.id, T1DURATION, T1LEVEL));
                    break;
                case 2:
                    player.addPotionEffect(new PotionEffect(Potion.resistance.id, T2DURATION, T2LEVEL));
                    break;
                case 3:
                    player.addPotionEffect(new PotionEffect(Potion.resistance.id, T3DURATION, T3LEVEL));
                    break;
                case 4:
                    player.addPotionEffect(new PotionEffect(Potion.resistance.id, T4DURATION, T4LEVEL));
                    break;
            }
            ...
            for(Object el : (world.getEntitiesWithinAABB(EntityPlayer.class,AxisAlignedBB.getBoundingBox(pos.posX-10,pos.posY-2,pos.posZ-10,pos.posX+10,pos.posY+2,pos.posZ+10)))) {
                switch(tier){
                    case 1:
                        ((EntityPlayer) el).addPotionEffect(new PotionEffect(Potion.resistance.id, T1DURATION, T1LEVEL));
                        break;
                    case 2:
                        ((EntityPlayer) el).addPotionEffect(new PotionEffect(Potion.resistance.id, T2DURATION, T2LEVEL));
                        break;
                    case 3:
                        ((EntityPlayer) el).addPotionEffect(new PotionEffect(Potion.resistance.id, T3DURATION, T3LEVEL));
                        break;
                    case 4:
                        ((EntityPlayer) el).addPotionEffect(new PotionEffect(Potion.resistance.id, T4DURATION, T4LEVEL));
                        break;
                }
            }
        }
        p_77659_1_.damageItem(1,player);
        world.playSoundAtEntity(player, "minst:horn", 1, 1);
