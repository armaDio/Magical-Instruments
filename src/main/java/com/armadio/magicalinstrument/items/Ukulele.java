...
public class Ukulele extends Item {
    ...
    public ItemStack onItemRightClick(ItemStack p_77659_1_, World world, EntityPlayer player) {
        if(!MinstConfiguration.areaEffect) {
            switch(tier){
                case 1:
                    player.addPotionEffect(new PotionEffect(Potion.waterBreathing.id, T1DURATION, T1LEVEL));
                    break;
                case 2:
                    player.addPotionEffect(new PotionEffect(Potion.waterBreathing.id, T2DURATION, T2LEVEL));
                    break;
                case 3:
                    player.addPotionEffect(new PotionEffect(Potion.waterBreathing.id, T3DURATION, T3LEVEL));
                    break;
            }
            ...
            for(Object el : (world.getEntitiesWithinAABB(EntityPlayer.class,AxisAlignedBB.getBoundingBox(pos.posX-10,pos.posY-2,pos.posZ-10,pos.posX+10,pos.posY+2,pos.posZ+10)))) {
                switch(tier){
                    case 1:
                        ((EntityPlayer) el).addPotionEffect(new PotionEffect(Potion.waterBreathing.id, T1DURATION, T1LEVEL));
                        break;
                    case 2:
                        ((EntityPlayer) el).addPotionEffect(new PotionEffect(Potion.waterBreathing.id, T2DURATION, T2LEVEL));
                        break;
                    case 3:
                        ((EntityPlayer) el).addPotionEffect(new PotionEffect(Potion.waterBreathing.id, T3DURATION, T3LEVEL));
                        break;
                }
            }
        }
        p_77659_1_.damageItem(1,player);
        world.playSoundAtEntity(player, "minst:ukulele", 1, 1);
